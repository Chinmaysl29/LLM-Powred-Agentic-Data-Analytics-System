// World map country path definitions and metadata
...
export const WORLD_MAP_COUNTRIES: WorldCountryShape[] = [
  {
    "id": "AD",
    "name": "Andorra",
    "region": "Europe",
  ...
  },
  {
    ...
    "name": "Zimbabwe",
    "region": "Africa",
    "shape": "M1148.2 713.7l6.2-7.2 1.6-4.6.9-.6.8-3.7-.8-1.9.5-4.7 1.3-4.4.3-8.1-2.8-2-2.6-.5-1.1-1.6-2.6-1.3-4.6.1-.3-2.4-4.2.9-3.3 2.5-.7 2.2-2 .5-5.1 5.3-3.2 4.1-1.9.1-1.7-.7-6.2-.7 1.9 5.1 1.1 1.1 1.6 3.7 6 7 2.3.7-.1 2.2 1.5 4.1 4.2.9 3.4 2.9 2.2.1 2.6 1.1 1.9-.8 2.9.6z"
  }
];

export const COUNTRY_BY_ID = new Map<string, WorldCountryShape>(
  WORLD_MAP_COUNTRIES.map((c) => [c.id, c])
);
...
export type GeoMatch =
  | { type: 'country'; id: string; country: WorldCountryShape }
  | { type: 'region'; region: string }
  | null;

export function normalizeGeoLocation(rawInput: unknown): GeoMatch {
  if (rawInput === null || rawInput === undefined) return null;
  const str = String(rawInput).trim().toLowerCase();
  if (!str) return null;

  // Check direct ISO code (case-insensitive)
  const upper = str.toUpperCase();
  if (COUNTRY_BY_ID.has(upper)) {
    return { type: 'country', id: upper, country: COUNTRY_BY_ID.get(upper)! };
  }

  // Check country alias
  const aliasId = COUNTRY_ALIASES[str];
  if (aliasId && COUNTRY_BY_ID.has(aliasId)) {
    return { type: 'country', id: aliasId, country: COUNTRY_BY_ID.get(aliasId)! };
  }

  // Check exact country name
  for (const country of WORLD_MAP_COUNTRIES) {
    if (country.name.toLowerCase() === str) {
      return { type: 'country', id: country.id, country };
    }
  }

  // Check region alias
  const matchedRegion = REGION_ALIASES[str];
  if (matchedRegion) {
    return { type: 'region', region: matchedRegion };
  }
...
/**
 * Monochromatic Pink Choropleth Palette
 * Seamless continuous RGB interpolation matching reference:
 * Dark muted rose -> Dark dusty pink -> Medium pink -> Light pink -> Bright rose-pink
 */
export function getPinkChoroplethColor(value: number, max: number): string {
  if (!Number.isFinite(value) || value <= 0 || max <= 0) {
    return '#1c1219'; // No sales data: subtle dark muted rose
  }

  const ratio = Math.min(1, Math.max(0, value / max));

  const stops = [
    { t: 0.0, r: 76, g: 28, b: 50 },     // #4c1c32: very low
    { t: 0.25, r: 122, g: 44, b: 79 },   // #7a2c4f: low
    { t: 0.5, r: 172, g: 68, b: 115 },   // #ac4473: medium
    { t: 0.75, r: 218, g: 110, b: 158 }, // #da6e9e: high
    { t: 1.0, r: 247, g: 168, b: 200 },  // #f7a8c8: very high / peak
  ];

  for (let i = 0; i < stops.length - 1; i++) {
    const s1 = stops[i];
    const s2 = stops[i + 1];
    if (ratio >= s1.t && ratio <= s2.t) {
      const f = (ratio - s1.t) / (s2.t - s1.t);
